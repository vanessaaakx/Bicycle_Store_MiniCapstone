                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12 mb-3" title="{{ $product->description }}">
                        <a href="#" class="card" href="" style="width: 100%; text-decoration:none;">
                            ...
                            <form method="post" action="{{ route('orders.create', $product->id) }}">
                                ...
                                <input type="text" class="form-control" id="status" value="Pending" name="status"
                                    hidden>
                                ...
                                <div class="card-body">
                                    ...
                                    <p><h6 class="card-text">{{ $product->category->name }}</h6></p>
                                    <h5 class="card-text"><strong>&#8369;{{ number_format($product->price, 2) }}</strong></h5>
                                    @if (!Auth::user())
                                        <button onclick='{{ url('/login') }}' class="btn btn-success my-2"><i
                                                class="bi bi-cart-plus"></i> Request order</button>
                                    @else
                                        <button type="submit" class="btn btn-success my-2"><i class="bi bi-cart-plus"></i>
